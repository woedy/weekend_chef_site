...
const UserDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch( baseUrl + `api/accounts/admin/get-user-details/?user_id=${user_id}`,  {
                headers: {
...
const handleVerifySudent = async (e) => {
  e.preventDefault();


  const url = baseUrl + 'api/accounts/verify-student/';
  const data = {
    'user_id': user_id,
  };
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      console.error('Verify failed:', responseData.message);
    }
  } catch (error) {
      ...
      <div className="overflow-hidden rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="relative z-20 h-35 md:h-65">
          ...
          <div className=" flex gap-2 absolute bottom-1 right-1 z-10 xsm:bottom-4 xsm:right-4">
            ...
            <label
                          onClick={handleVerifySudent}

              htmlFor="cover"
              className="flex cursor-pointer items-center justify-center gap-2 rounded bg-primary py-1 px-2 text-sm font-medium text-white hover:bg-opacity-90 xsm:px-4"
            >
             
             <span>{ user.verified === true ? 'Verified' : 'Verify'}</span>
             </label>
        ...
        <div className="relative z-30 mx-auto -mt-22 h-32 w-32 max-w-32 rounded-full bg-white/20 p-1 backdrop-blur sm:h-44 sm:w-44 sm:max-w-44 sm:p-3">
  ...
  <p className="font-medium">{user.year_group}</p>
  <p className="font-medium">{user.user_profile?.house || 'N/A'}</p>

  <div className="mx-auto max-w-180 mt-3">
    <h4 className="font-semibold text-black dark:text-white">
      About Me
    </h4>
    <p className="mt-4">{user.about_me || 'No information available'}</p>
  </div>

  <h2 className="mb-1.5 mt-5 text-xl font-semibold text-black dark:text-white">
    General Info
  </h2>
  ...
  <div className='grid grid-cols-4 '>
    <div className="mx-auto max-w-180 mt-3">
      ...
      <p className="mt-4">{user.user_profile?.profession || 'N/A'}</p>
    </div>
    ...
    <div className="mx-auto max-w-180 mt-3">
      ...
      <p className="mt-4">{user.user_profile?.job_title || 'N/A'}</p>
    </div>
    ...
    <div className="mx-auto max-w-180 mt-3">
      ...
      <p className="mt-4">{user.user_profile?.place_of_work || 'N/A'}</p>
    </div>
    ...
    <div className="mx-auto max-w-180 mt-3">
      ...
      <p className="mt-4">{user.user_profile?.city || 'N/A'}</p>
    </div>
  </div>
  ...
  {user.user_interests && user.user_interests.length > 0 ? (
          user.user_interests.map((interest, index) => (
            <div key={index} className="bg-primary text-white py-1 px-2 rounded text-sm font-medium">
              {interest}
            </div>
          ))
        ) : (
  ...
  <div className="flex flex-wrap gap-4  items-center justify-center mt-4">
        {user.user_photos && user.user_photos.length > 0 ? (
          user.user_photos.map((photo, index) => (
